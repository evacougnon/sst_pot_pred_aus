...
fname_pp = '/home/ecougnon/ana/PotPred/PP_SSTa_monthly_1yr_ZFL_Aus.nc'
fname_eof = '/home/ecougnon/ana/PotPred/EOF/cov_decomp_1yr_ZFL_EOF_Aus_1deg_TMM'
figfile = '/home/ecougnon/ana/PotPred/EOF/EOF4var_in_PotPred.png'
...
PP = PP.sel(lat =slice(lat_min,lat_max,lat_bin), \
...
var_eof = MODES_MAP['pred_modes_map'][-4,:,:]
var_PC = PC['PC4_pred'].copy()
...
proj.drawmeridians(range(domain_draw[1],domain_draw[3]+1,dlon), \
                   labels=[False,False,False,True], fontsize=14)
lonproj, latproj = proj(llon, llat)
plt.contourf(lonproj, latproj, var_ratio, levels= lev_pp, cmap=plt.cm.afmhot_r)
cb=plt.colorbar(ticks = lev_pp, shrink=0.9)
plt.title('VAR(mode4_pred)/VAR(tot)', fontsize=16, y=1.04)
...
proj.drawmeridians(range(domain_draw[1],domain_draw[3]+1,dlon), \
                   labels=[False,False,False,True], fontsize=14)
lonproj, latproj = proj(llon, llat)
plt.contourf(lonproj, latproj, (1-(var_pp-var_ratio))*100, levels= lev, \
             cmap=plt.cm.afmhot_r) #seismic)
cb=plt.colorbar(ticks = lev_, shrink=0.9)
plt.title('% of PP explained by mode 4', fontsize=16, y=1.04)
