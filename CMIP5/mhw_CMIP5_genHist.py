...
for model in ['ACCESS1-3', 'CSIRO-Mk3-6-0', 'CNRM-CM5', 'HadGEM2-ES', 'IPSL-CM5A-LR', 'IPSL-CM5A-MR', 'CanESM2']:
    if model == 'ACCESS1-3':
    ...
    for run in range(NRUNS):
        for ens in range(NENS[run]):
            print model, 'Run', run, 'Ensemble member', ens
            # Files
            files = []
    ...
    outfile = '/data/MHWs/BAMS/CMIP5/mhw_' + region + '_' + model + '_hist'
